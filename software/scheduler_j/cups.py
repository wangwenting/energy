"""
cups (Random) Scheduler implementation, data gathering is a periodictiy task by energy-bmc
rank the servers base on cups index maxinum to mininum
choose the mininum server
"""
...
CONF = cfg.CONF
CONF.register_opts(cups_opts)
CONF.import_opt('compute_topic', 'nova.compute.rpcapi')


class CupsScheduler(driver.Scheduler):
    """Implements Scheduler as a random node selector."""

    def __init__(self, *args, **kwargs):
        self.compute_rpcapi = compute_rpcapi.ComputeAPI()
        super(CupsScheduler, self).__init__(*args, **kwargs)
    ...
    def cups_low_workload_host_select(self):
        try:
            ret = self.con.cups_low_workload_host_select()
        except Exception, e:
            self.con.connect()
    ...
    def _schedule(self, context, topic, request_spec, filter_properties):
        ...
        LOG.debug("WTWANG After Filter HOSTS %s" % hosts)
        if not hosts:
        ...
        ret = self.cups_low_workload_host_select()
        if ret.value:
            ret = json.loads(ret.value)
        else:
            return None
        LOG.info('WTWANG ret of cups_host_select = %s' % ret)
        if len(ret) > 0:
            hosts = ret
    ...
    def schedule_run_instance(self, context, request_spec,
        ...
        LOG.debug("WTWANG scheduler_run_instance start instance_uuid:%s" % instance_uuids)
